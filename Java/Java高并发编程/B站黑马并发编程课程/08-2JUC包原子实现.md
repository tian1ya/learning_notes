...
> 主要是使用在如下得场景中：
>
> 主线程需要开启多个线程并行取执行任务，并且主线性簇拥等待所有子线程执行结束之后再进行汇总。
>
> `countDownLatch` 出现之前均是使用`join` 方法实现这点，但是`join` 不够灵活，不能够满足不同场景得需要。
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        Thread thread2 = new Thread(() -> {
            try {
                ...
                // 内部计数器 -1， 当计数器=0得时候，主线程 await 方法会返回，释放主线程
                countDownLatch.countDown();
            }
            System.out.println("child thread over!");
        });
...
再一般使用线程得场景中均是使用线程池，这个时候就不能使用`join` 了。需要使用`CountDownLatch`。
