...
* 为什么选择Flink
  ...
  >
  > （为了攒够一批数据去处理，必然会遇到延迟的问题，Flink 主要也就是解决这个问题，不会去等一批数据去处理，而是会来一条处理一条，或者没秒来n条，然后处理n条数据）
  >
  > 我们的目标是
  >
  > * 低延迟
...
* Flink on Yarn 
  ...
  > Flink 提供了2中在yarn 上运行的模式，分别为Session-Cluster 和 Per-Job-Cluster 模式
  >
  > * Session-cluster
  >
  > ![a](./pic/fy.png)
  ...
  >   ![a](./pic/fy1.png)
  >
  >   一个Job 会对应一个集群，每提交一个作业会根据自身的情况，都会单独向 yarn 申请资源，知道作业执行完，一个作业的失败与否不会影响到下一个作业的正常提交和运行，独享Dispather 和 ResourceManager，按需接收资源申请，适合规模大长时间运行的作业。
  >
...
* **Flink 运行时组件(Runtime)
  ...
  > 整体流程：
  >
  > > JM 向RM 申请资源，RM会安全TM中的资源去响应JM资源请求，然后JM将任务分发到JM的资源上
  >
  ...
  > ![a](./pic/fr2.png)
  >
  > Dispacher: 提供 REST 接口，用的少，提供Rest应用的提交，一家web页面
  >
  > ![a](./pic/fr3.png)
...
* 数据流DataFlow
  ...
  Flink 中的Job 是如何划分的？ 为什么有时候将几个算子chaiin起来变为一个Job，而有的一个算子是一个Job
...
>
> * source: 处理提供的一些特定输入的类，还可以自定义source， 实现SourceFuntion
>
>   ```scala
...
>   ```
>
>   Reduce：比max 等聚合更加的一般化
>
>   底层都是调用了aggeration 方法
>
...
>
> **窗口就是讲无限流切割为有限流的一种方式，它会将流数据分发到有限大小的桶中进行分析**
>
> * 窗口类型
>
...
> ![a](./pic/ft44.png)
>
> 然后来了5这个数据，计算wm 5-3=2，表示时间戳小于2的数据都已经到了，可以关闭[0,2)这个时间窗口了，将4放入到[5,10)桶中，因为没有[0,2)这个时间窗口，所以没有关闭时间窗口的操作。
>
> ![a](./pic/ft55.png)
>
...
> // 这个时候时间点还没过了1min，所以窗口还没有关闭，窗口还会等待1min
...
> // 228-3=225 这个时候触发在一次的窗口背景，触发逻辑计算
> result> (sensor_1,32.0,1547718208)
> result> (sensor_7,6.7,1547718202)
> result> (sensor_6,15.4,1547718201)
> result> (sensor_10,38.1,1547718205)
...
> // 这个时候继续输出了，这是因为当前的wm 已经到了 282
> // 以及超过了窗口 [225,240) 且这个窗口内是有数据的所以触发了
...
> ![a](./pic/fsql27.png)
>
> Table 中认为批处理是流处理的特列。其底层均是 DataStream，Table 处理流或者批都是使用相同的模式，
>
> ![a](./pic/fsql.png)
>
> ![a](./pic/fsql1.png)
