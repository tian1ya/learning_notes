...
**Tolerance（容错）**：在集群出现分区时，整个系统也要持续对外提供服务，在容错的时候，访问01和访问03，获取到的数据是不一致的，因为01中的数据由于分区问题，数据并没有及时同步过去。



### 2.1.4.矛盾

在分布式系统中，系统间的**网络不能100%保证健康，一定会有故障的时候**，而服务有必须对外保证服务。因此Partition Tolerance不可避免。
...
也就是在CAP的A和C两点做妥协，P + 部分A + 最终C，**架构设计：在各个技术中取平衡**
...
- CP模式：各个子事务执行后互相等待，**同时提交，同时回滚，达成强一致**。但事务等待过程中，处于弱可用状态。
  ...
  等待的事务会有锁存在，影响数据库的吞吐和性能，典型的解决方案有 **2PC**
...
- **TC (Transaction Coordinator) -** **事务协调者，全局的决策者：**维护全局和分支事务的状态，协调全局事务提交或回滚。

- **TM (Transaction Manager) -** **事务管理器，全局的执行者：**定义全局事务的范围、开始全局事务、提交或回滚全局事务。

- **RM (Resource Manager) -** **资源管理器，局部的管理&执行者：**管理分支事务处理的资源，与TC交谈以注册分支事务和报告分支事务的状态，并驱动分支事务提交或回滚，一个RM对应一个局部数据库事务。

TM 维护者全局的事务，RM 给 TC汇报每个局部事务的状态，TC 根据RM汇报的每个布局事务的状态，对全局事务的操作进行决策，TC决策后，决策结果有TM执行 。

TM和RM是对业务方法的一种代码，和业务代码是在一起的。TC是独立于业务代码，独立部署的。在RM给TC汇报每个节点的事务状态的时候是需要和TC进行通信的，那么最好的方式是通过注册中心，而不是直连的方式，所以在运行TC的时候，是还需要一个注册中心
...
- 因为一阶段需要锁定数据库资源，等待二阶段结束才释放，性能较差，在第一段和第二段提交直接发送网络异常，那么数据库锁会无法释放，直接锁死
- 依赖关系型数据库实现事务，所以如果使用非事务型数据库，就不能使用这种方法
...
- 注册分支事务
- 记录undo-log（RM拦截执行sql生成数据快照，AT 模式需要 `UNDO_LOG` 表 ）
- 执行业务sql并提交(不存在事务等待，也就没有XA中的死锁/锁表以及吞吐降低的问题)
- 报告事务状态
...
- 根据undo-log恢复数据到更新前，恢复后再删除undo-log
...
上面的问题，在恢复数据的时候，直接从事务2的结果80恢复到事务1的快照100，也就是事务1更新为90的提交了的事务事情就白做了，将更新丢失。

解决思路就是引入了**全局锁**的概念。在释放DB锁之前，先拿到全局锁。避免同一时刻有另外一个事务来操作当前数据。

> 所有事务分支操作都去抢一把锁，抢到锁才能操作。包括全局事务的提交也得等子事务把这把锁释放掉才能操作

当前事务1，正在持有表 account 表的第1行数据的锁，
...
- 两阶段之间属于软状态，属于最终一致
- 框架的快照功能会影响性能，但比XA模式要好很多，全局锁是seata维护的，只是不能修改行，还是可以读的，而XA的事务是数据库维护的，枷锁的时候，CRUD均不可以。
...
只不过，AT模式需要一个表来记录全局锁、另一张表来记录数据快照seata.undo_log。



1）导入数据库表，记录全局锁

导入课前资料提供的Sql文件：seata-at.sql，其中seata_demo.lock_table导入到TC服务关联的数据库，seata.undo_log表导入到微服务关联的数据库：
...
XA 和 AT模式都是要加锁，其性能方面总是会有损坏，如果要追求极致的性能，那么就需要考虑下面的不加锁的方式
...
- Try：资源检查和预留
- Confirm：预留资源上做业务执行和提交
- Cancel：预留资源的释放

TCC的优点是什么？

- 一阶段完成直接提交事务，释放数据库资源，性能好
- 不依赖数据库，基于补偿，所以数据库有没有事务能力无所谓，相比AT模型，无需生成快照，无需使用全局锁，性能最强
...
- 软状态，事务是最终一致
- 需要考虑Confirm和Cancel的失败情况，会有重试情况，有重复操作的可能，所以要做好**幂等处理**

### 4.3.4.事务悬挂和空回滚

#### 1）空回滚

当某分支事务的try阶段**阻塞**时，可能导致全局事务超时而触发二阶段的cancel操作。**在未执行try操作时先执行了cancel操作，这时cancel不能做回滚，就是空回滚**。
...
> TCC 和上面XA/AT的模式类似于乐观锁和悲观锁的区别
>
> TCC在额外的非业务代码逻辑去判断事务性，以及是否做回滚/重试等，和乐观锁相似。
>
> XA/AT 就好比`synchronize` 直接给资源上锁，独占，我昨晚了在释放锁，其他操作才可以进行操作
...
> [长事务]([MySQL-长事务详解 - MySQL技术 - 博客园 (cnblogs.com)](https://www.cnblogs.com/mysqljs/p/11552646.html) ): 就是运行时间比较长，长时间未提交的事务，也可以称之为大事务。这类事务往往会造成大量的阻塞和锁超时，容易造成主从延迟，要尽量避免使用长事务。 
>
> [官方最佳实践]([Seata Saga 模式](https://seata.io/zh-cn/docs/user/saga.html) )
...
1. 第一阶段直接提交本地事务(这是和TCC的不一样的地方)
2. 第二阶段中，如果第一阶段所有事务均成功，那么什么都不需要做，如果有任何一个失败，那么执行每个操作的补充操作，将第一阶段的操作进行回滚(不需要做冻结资源的是否，直接补偿回滚)

和TCC相似，但是比TCC简单，无额外资源辅助操作。**没有全局锁，没有额外资源做保护，缺少事务隔离**
