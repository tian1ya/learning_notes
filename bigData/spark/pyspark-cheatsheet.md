...
### spark 算众数https://stackoverflow.com/questions/36654162/mode-of-grouped-data-in-pyspark/36695251#36695251

```python
import numpy as np
rand_values = np.random.randint(3,size=10).reshape((5, 2))

df = spark.createDataFrame(rand_values.tolist(), pst.StructType([pst.StructField("x", LongType(), True), pst.StructField("y", LongType(), True)]))

counts = df.groupBy(['x', 'y']).count().alias('counts')

result = counts \
          .groupBy('x') \
          .agg(psf.max(psf.struct(psf.col('count'),psf.col('y'))).alias('max')) \
          .select('x', psf.col('max.y'))
